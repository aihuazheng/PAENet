...
gallset = TestData(gall_img, gall_label,gall_atr,transform=transform_test, img_size=(args.img_w, args.img_h))
...
if args.optim == 'sgd':
    if args.pcb == 'on':
        ...
        ignored_params = list(map(id, net.bottleneck.parameters())) \
                       + list(map(id, net.class_8.parameters()))\
                       + list(map(id, net.class_0.parameters()))\
                       + list(map(id, net.class_1.parameters()))\
                       + list(map(id, net.class_2.parameters()))\
                       + list(map(id, net.class_3.parameters()))\
                       + list(map(id, net.class_4.parameters()))\
                       + list(map(id, net.class_5.parameters()))\
                       + list(map(id, net.class_6.parameters()))\
                       + list(map(id, net.class_7.parameters()))\
                       + list(map(id, net.ASENet.parameters()))\
                       + list(map(id, net.cross_transformer1.parameters()))
        ...
        optimizer = optim.SGD([
            ...
            {'params': net.class_8.parameters(), 'lr': args.lr},
            {'params': net.class_0.parameters(), 'lr': args.lr},
            {'params': net.class_1.parameters(), 'lr': args.lr},
            {'params': net.class_2.parameters(), 'lr': args.lr},
            {'params': net.class_3.parameters(), 'lr': args.lr},
            {'params': net.class_4.parameters(), 'lr': args.lr},
            {'params': net.class_5.parameters(), 'lr': args.lr},
            {'params': net.class_6.parameters(), 'lr': args.lr},
            {'params': net.class_7.parameters(), 'lr': args.lr},
            {'params': net.ASENet.parameters(), 'lr': args.lr},
            {'params': net.cross_transformer1.parameters(), 'lr': args.lr}],
            weight_decay=5e-4, momentum=0.9, nesterov=True)
...
def train(epoch):
    ...
    for batch_idx, (input1, input2, label1, label2,atr1,atr2) in enumerate(trainloader):
        ...
        if args.pcb ==  'on':
            feat, out0, feat_all = net(input1, input2,atrs)  
            ...
            feat, out0 ,att= net(input1, input2,atrs)
            loss_id = criterion_id(out0, labels)
            loss_atr = criterion_bce(att, atrs)
            loss_tri, batch_acc = criterion_tri(feat, labels)
            correct += (batch_acc / 2)
            _, predicted = out0.max(1)
            correct += (predicted.eq(labels).sum().item() / 2)
            loss =  loss_id + loss_tri * args.w_center  + loss_atr# 
...
def test(epoch):
    ...
    with torch.no_grad():
        for batch_idx, (input, label,atr_label) in enumerate(gall_loader):
            ...
            if args.pcb == 'on':
                feat = net(input, input, atr_label,test_mode[0])
                gall_feat[ptr:ptr + batch_num, :] = feat.detach().cpu().numpy()
            else:
                feat, feat_att = net(input, input, atr_label,test_mode[0])
                gall_feat[ptr:ptr + batch_num, :] = feat.detach().cpu().numpy()
    ...
    with torch.no_grad():
        for batch_idx, (input, label,atr_label) in enumerate(query_loader):
            ...
            input = Variable(input.cuda())
            if args.pcb == 'on':
                feat = net(input, input, atr_label,test_mode[1])
                query_feat[ptr:ptr + batch_num, :] = feat.detach().cpu().numpy()
            else:
                feat, feat_att = net(input, input, atr_label,test_mode[1])
                query_feat[ptr:ptr + batch_num, :] = feat.detach().cpu().numpy()
